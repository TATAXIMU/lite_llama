...
class TestLlamaRotaryEmbedding(unittest.TestCase):
    ...
    def test_default_rope_parameters(self):
        ...
        inv_freq, attention_scaling = rotary_emb.rope_init_fn(
            None, torch.device("cpu"), **rotary_emb.rope_kwargs
        )
        self.assertEqual(
            inv_freq.shape[0], self.config.head_dim // 2
        )  # dim=4, step=2 -> 2
    ...
    def test_llama3_rope_parameters(self):
        ...
        inv_freq, attention_scaling = rotary_emb.rope_init_fn(
            self.config, torch.device("cpu"), **rotary_emb.rope_kwargs
        )
    ...
    def test_forward_output_shape(self):
        ...
        position_ids = (
            torch.arange(0, seq_length).unsqueeze(0).expand(batch_size, seq_length)
        )
